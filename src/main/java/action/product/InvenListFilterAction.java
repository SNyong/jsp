...
import svc.product.InvenListFilterService;
...
public class InvenListFilterAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (id == null) { // 로그인 했을시 사용(admin 권한)
			...
			InvenListFilterService invenListFilterService = new InvenListFilterService();
			int listCount = invenListFilterService.getListCount(Code);
			...
			ArrayList<Inventory> invenList = invenListFilterService.getinvenListFilter(Code,page, limit );
			request.setAttribute("invenList", invenList);
			request.setAttribute("pageInfo", pageInfo);
			request.setAttribute("Code", Code);
			request.setAttribute("pagefile", "./admin.jsp");
			request.setAttribute("pagefile2", "./product/invenListFilter.jsp");
